...
def web3_call(fn, *args, label="web3", max_retries=3, **kwargs):
    for attempt in range(1, max_retries + 1):
        try:
            ...
            err = str(e)
            is_conn_err = any(k in err for k in ['10054', 'ConnectionReset', 'Connection aborted', 'RemoteDisconnected', 'ConnectionError'])
...
def send_tx_with_retry(signed_tx, label="TX", max_retries=3):
    for attempt in range(1, max_retries + 1):
        try:
            ...
            err_str = str(e)
            is_conn_err = any(k in err_str for k in ['10054', 'ConnectionReset', 'Connection aborted', 'RemoteDisconnected'])
            if is_conn_err:
                wait = 5 * attempt
...
def node_web3_executor(state: AgentState) -> dict:
    print("\n[WEB3 TRANSACTION] Acionando Cofre na Sepolia Testnet...")
    
    if not PRIVATE_KEY or not TREASURY_CONTRACT_ADDRESS:
        print("Erro: Chaves e Contrato nao configurados. Abortando execucao Web3 real.")
        return {"tx_hash": "error_missing_config", "events": [{"sender": "Sistema", "message": "Configuracao Web3 ausente.", "type": "error"}]}

    try:
        contract = w3.eth.contract(address=TREASURY_CONTRACT_ADDRESS, abi=TREASURY_ABI)
        recipient = state.get("recipient_wallet_address")
        amount = state.get("grant_amount")
        ...
        tx = contract.functions.submitGrant(
            recipient, 
            amount
        ).build_transaction({
            'chainId': chain_id,
        ...
        print(f"Erro Web3 Engine: {e}")
        traceback.print_exc()
        events = [{"sender": "Sistema", "message": f"ERRO DE TRANSACAO WEB3: {e}", "type": "error"}]
        return {"tx_hash": "error", "events": events}

# --------------------------
# 6. Grafo (Edges e Routing)
# --------------------------
